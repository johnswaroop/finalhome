...
const PowerBtn = ({ setSwitchData, switchData, socket }) => {
  ...
  useEffect(() => {
    let urlNew = window.localStorage.getItem('ip');
    seturl(urlNew);
    console.log(urlNew);
  }, [])
  ...
  useEffect(() => {
    const add = localStorage.getItem(`ip`);
    console.log(add);
  }, []);
  return (
    <div
      ...
      onClick={() => {
        setOn(!on);
        if (switchData[socket].status == true) {
          axios.get(`${url}/${socket[1]}/off`);
        }
        else if (switchData[socket].status == false) {
          axios.get(`${url}/${socket[1]}/on`);
        }
      }}
    >
      <span></span>
...
const EditPopUp = ({
  ...
  useEffect(() => {
    setForm({
      label: switchData[popVisible.socket].label,
      icon: switchData[popVisible.socket].icon,
    });
  }, []);
    ...
    <div className={styles.editPopUp}>
      <span>
        <button
          ...
          onClick={() => {
            setPopVisible({ visisble: false, socket: null });
          }}
        >
          ❌
        </button>
          ...
          if (switchData[key].inUse == false) {
            return (
              <span
                ...
                onClick={() => {
                  setPopVisible({ socket: key, visisble: true });
                }}
              >
